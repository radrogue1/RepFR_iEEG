...
from mne import channels, Epochs, time_frequency
...
def Create_Epoch(ep, events, start, stop):
    '''
...
# #     print("maybe here?")
#     ep = Epochs(eeg, mne_events, tmin =  start, tmax = stop, baseline = None, preload = True, on_missing = 'ignore', verbose = False)
...
def Channel_Powers(subject, session, channels, eeg, buffer_minimized, wave_number, freqs, path, error_log_path, log = True):
...
#         wave_pows = MorletWaveletFilter(channel_eeg, freqs = freqs, width = wave_number, output = 'power').filter()
        ...
        i_wave_pows = wave_pows.remove_buffer(buffer_minimized)
        if log == True:
...
def get_enc_powers(subject, pairs, hemisphere, region):
    ...
    for sess in sessions:
        try:
            if subject == 'R1566D' and sess == 1:
                pass
            print(subject, sess, 'loading...')
            r = CMLReader(subject=subject, experiment='RepFR1', session=sess)
            ...
            eeg = r.load_eeg(events=word_evs, rel_start= rel_start + (-1 * buffer_ms), rel_stop=1600 + buffer_ms, scheme=pairs, clean=False)
            eeg = eeg.resample(500.)
            ...
            enc_x = eeg.to_ptsa()
            print(path)
    #         ep, enc_x = Create_Epoch(eeg.to_mne(), word_evs, rel_start + (-1 * buffer_minimized), rel_stop + buffer_minimized)
            if np.all(enc_x == 0):
            ...
            Channel_Powers(subject = subject, session = sess, channels = channels, eeg=enc_eeg, buffer_minimized=buffer_minimized, wave_number = wave_number, freqs=freqs, path=path, error_log_path = '', log = True)
...
def enc_power_statistics(subject, pairs, hemisphere, region):
    ...
    for sess in sessions:
        ...
        try:
            ...
            # *** once presented items, recalled, not recalled *** #
            pres1_evs = word_evs[(word_evs.repeats==1) & (word_evs.list != 0)]
            pres1_recalled = pres1_evs.recalled.to_numpy()
            pres1_forgotten = ~pres1_recalled
            
            # *** Novel and Repeat presentations of thrice presented words *** #
            f_evs = word_evs[(word_evs.repeats == 3) & (word_evs.list != 0)]
            ...
            # *** All presentations of thrice presented words *** #
            three_recalled = f_evs.recalled.to_numpy()
            ...
            pres1 = session_d[:, :, pres1_evs.index, :]
            pres1_forgot = pres1[:, :, pres1_forgotten, :]; subj_pres1_n.append(pres1_forgot)
            pres1_rec = pres1[:, :, pres1_recalled, :]; subj_pres1_r.append(pres1_rec)
            del pres1; del pres1_forgot; del pres1_rec;
            ...
            three_presentation = session_d[:, :, f_evs.index, :]
    ...
    session_pres1N = np.concatenate(subj_pres1_n, axis = 2)
    del subj_pres1_n;
    session_pres1R = np.concatenate(subj_pres1_r, axis = 2)
    del subj_pres1_r;        
    ...
    print("running stats")
    def check_tvals(tvals):
        check_tvals = abs(tvals)
        if check_tvals[check_tvals>7].shape[0] > 0:
            return False
        return True
    
    failed_tval_check = []
    # *** One Presentation Remembered - Forgotten *** #
    pres1_tvals = stats.ttest_ind(session_pres1R, session_pres1N, equal_var = False, axis = 2).statistic  
    if check_tvals(pres1_tvals):
        pres1_x = xr.DataArray(pres1_tvals, dims = ['channels', 'freqs', 'timepoints'], 
                      coords = {'channels': select_channels})
    else: 
        failed_tval_check.append('Pres1 SME')
        pres1_x = []
    ...
    file_name = '{}_{}_sme_tscore'.format(subj, "pres1")
    save_numpy(pres1_x, save_path, file_name)
    ...
    print("data saved.")
    print("\n")
    if len(failed_tval_check) == 0:
        return (subj + ' worked!')
    else:
        return failed_tval_check


# check_pres1_tvals = abs(pres1_tvals)
#     if check_pres1_tvals[check_pres1_tvals>7].shape[0] > 0:
#         raise Exception('A channel for once presented item sme has a t-value over 7! Bad.')